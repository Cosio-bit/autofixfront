...
const ReparacionList = () => {
  ...
  const { tipoMotor } = useParams();
  const navigate = useNavigate();

  useEffect(() => {
    init(tipoMotor); // Pasamos el parámetro tipoMotor cuando está presente
  }, [tipoMotor]); // Dependencias del efecto
  
  const init = () => {
    //si es numero entonces es idVehiculo, si no es tipoMotor
    
      if (tipoMotor) {
      reparacionService
        .getByTipoMotor(tipoMotor)
        .then((response) => {
          console.log(`Mostrando listado de todas las reparaciones de la tipoMotor ${tipoMotor}.`, response.data);
          setReparaciones(response.data);
        })
        .catch((error) => {
          console.error(
            `Se ha producido un error al intentar mostrar listado de todas las reparaciones de la tipoMotor ${tipoMotor}.`,
            error
          );
        });
    } else {
